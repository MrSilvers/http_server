...
int main(int argc,char const *argv[]){
    ...
    sigaddset(&initmask, SIGPIPE);
    if(pthread_sigmask(SIG_BLOCK, &initmask, NULL)!=0){
        perror("set initmask");
    }
    while(1){
        int numbytes = 0;
        int recv_times = 1;
        char *msg = NULL;
        struct pthread_argv args;
        pthread_t pid;

        if((request_str = (char *)malloc(MAX_RECV_LEN+1)) == NULL){
            perror("malloc request_str");
            exit(1);
        }
        ...
        if(request_size >= 3+1+1+1+8+2+4){
            *(request_str+request_size) = '\0';
            ...
            if(pthread_create(&pid, NULL, (void *)request_handle, (void *)&args)!=0){
                perror("pthread_create");
            }
            pthread_join(pid, &msg);
...
char* request_handle(void *a){
    struct pthread_argv *args = (struct pthread_argv *)a;
    int client_sockfd = args->sockfd;
    char *request_str = args->request;
    DEBUG("request_handle:request_str: %s", request_str);
    DEBUG("request_handle:client_sockfd: %d", client_sockfd);
    ...
    */
    ...
    char *buf;
    if((buf = (char *)malloc(strlen(request_str)+1))==NULL){// strlen的结果不包含'/0'
        perror("request_handle malloc buf");
        exit(1);
    }
...
char *post_handle(int client_sockfd,struct request *request){
    ...
    DEBUG("cgi_program path:%s",filename);
